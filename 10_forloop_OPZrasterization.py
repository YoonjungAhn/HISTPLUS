...
out_surface_BUA = np.zeros((len(x_range),len(y_range))).astype(np.float32)


years=range(1820,2025,5)
...
for year in years:
    ...
    for name in glob.glob('D:/DATA/01_ParcelOCMZtraxMerge/*.csv'):
        ...
        curr_surface = scipy.stats.binned_statistic_2d(gpdf.geometry.x.values,gpdf.geometry.y.values,statsvals,statistic=statistic_str,bins = [xbins, ybins],range=rasterrange)        
        out_surface_BUA = np.maximum(out_surface_BUA,np.nan_to_num(curr_surface.statistic))       
        out_surface_BUA[out_surface_BUA> 1] = 1

    gdalNumpy2floatRaster_compressed(np.rot90(out_surface_BUI),'D:/DATA/Raster/BUI/'+str(year)[0:-2]+'_counties_resident_surface_%s_%s.tif' %(target_variable1,statistic_str),template_raster,cols,rows,bitdepth)
    gdalNumpy2floatRaster_compressed(np.rot90(out_surface_BUPR),'D:/DATA/Raster/BUPR/'+str(year)[0:-2]+'_counties_resident_surface_%s_%s.tif' %(target_variable2,statistic_str),template_raster,cols,rows,bitdepth)  
    gdalNumpy2floatRaster_compressed(np.rot90(out_surface_BUA),'D:/DATA/Raster/BUA/'+str(year)[0:-2]+'_counties_resident_surface_%s_%s.tif' %(target_variable3,statistic_str),template_raster,cols,rows,bitdepth)
